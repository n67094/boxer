...
ENGINE_INLINE char *
engine_io_read(const char *path, size_t *length)
{
  SDL_assert(path);
  ...
  if (!file) {
    ...
    engine_set_error(ENGINE_ERROR_IO_READ);
    return NULL;
  }
...
ENGINE_INLINE bool
engine_io_write(const char *path, const void *data, size_t length, bool append)
{
  SDL_assert(path);
  ...
  if (!file) {
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                ...
                path,
                PHYSFS_getLastErrorCode());
    engine_set_error(ENGINE_ERROR_IO_WRITE);
    return false;
  }
