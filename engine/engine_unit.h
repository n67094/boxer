...
typedef bool (*engine_unit_case_cb)(void);
typedef void (*engine_unit_suite_cb)(void);

bool engine_unit_assert(bool passed,
                        const char *const expr,
                        const char *const file,
                        int line);

void engine_unit_run_case(const char *const name, engine_unit_case_cb case_cb);

void engine_unit_run_suite(const char *const name,
                           engine_unit_suite_cb suite_cb);
 ...
 */
#define ENGINE_UNIT_CASE(name) static bool name(void)
 ...
 */
#define ENGINE_UNIT_ASSERT(expr)                                               \
  do {                                                                         \
    if (!engine_unit_assert(                                                   \
            (expr) ? true : false, (#expr), __FILE__, __LINE__))               \
      return false;                                                            \
  } while (false)
 ...
 */
#define ENGINE_UNIT_RUN_CASE(test_cb) (engine_unit_run_case(#test_cb, test_cb))
 ...
 */
#define ENGINE_UNIT_SUITE(name) void name(void)
 ...
 */
#define ENGINE_UNIT_RUN_SUITE(suite_cb)                                        \
  engine_unit_run_suite(#suite_cb, suite_cb)

/**
 * @brief Functions that are run before and/or after a number of unit
 * tests execute.
 */
typedef void (*engine_unit_setup_cb)(void);
