...
typedef struct engine_game_config_s
{
...
/**
 * @brief The user should provide this function to return the engine game
 * configuration.
 *
 * @return A pointer to the engine DEFAULT game configuration structure.
 *
 * @note If a file named "config.ini" exists in the PHYSFS mount directories,
 * the engine will override the default configuration values with the values
 * from the INI file.
 *
 ...
 * - target_ups
 */
extern engine_game_config_t *engine_game_config(void);
 ...
 * engine has been initialized and the window has been created. You can use this
 * function to load assets, initialize game state, etc.
 */
extern void engine_game_setup(void);
 ...
 * handling input, etc.
 */
extern void engine_game_update(Uint64 delta_time_ms);
 ...
 */
extern void engine_game_render(Uint64 alpha_time_ms);
 ...
 * game state, etc.
 */
extern void engine_game_shutdown(void);
