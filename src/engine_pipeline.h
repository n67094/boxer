...
typedef enum
{
  ENGINE_BLENDMODE_NONE                = SDL_BLENDMODE_NONE,
...
typedef enum
{
  ...
  ENGINE_PRIMITIVE_POINTS         = SDL_GPU_PRIMITIVETYPE_POINTLIST,
  ENGINE_PRIMITIVE_SIZE           = 5,
} engine_primitive_e;
...
/**
 * @brief Attach an SDL pipeline to an engine pipeline. Usefull to use custom
 * pipelines with the painter.
 */
engine_pipeline_t engine_pipeline_attach(SDL_GPUGraphicsPipeline *pipeline);
